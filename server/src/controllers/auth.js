...
async function register(req, res) {
  ...
  const user = await User.create({ name, email, passwordHash, grade, role: role || 'user', xp: 0, level: 1, badges: [] });
...
async function me(req, res) {
  ...
  res.json({ name: user.name, id: user._id, email: user.email, grade: user.grade, xp: user.xp, level: user.level, badges: user.badges, role: user.role });
}
